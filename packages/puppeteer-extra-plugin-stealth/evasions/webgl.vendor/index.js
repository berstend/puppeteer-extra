  ...
  async onPageCreated(page) {
    if (!this._session) {
      ...
      if (userAgent.includes('(windows')) {
        ...
        this._session.s37446 =
          this.opts.renderer ||
          WEBGL_RENDERERS_DESKTOP[
            Math.floor(Math.random() * WEBGL_RENDERERS_DESKTOP.length)
          ]
      } else if (userAgent.includes('(android')) {
        this._session.s37445 =
          this.opts.vendor ||
          WEBGL_VENDORS_ANDROID[
            Math.floor(Math.random() * WEBGL_VENDORS_ANDROID.length)
          ]
        this._session.s37446 =
          this.opts.renderer ||
          WEBGL_RENDERERS_ANDROID[
            Math.floor(Math.random() * WEBGL_RENDERERS_ANDROID.length)
          ]
        ...
        this._session.s37446 =
          this.opts.renderer || 'Intel HD Graphics 4000 OpenGL Engine'
      } else {
        this._session.s37445 = this.opts.vendor || vendor
    ...
    await withUtils(page).evaluateOnNewDocument(
      (utils, opts, session) => {
        ...
        const bufferDataProxyHandler = {
          apply: function(target, ctx, args) {
            for (let i = 0; i < args[1].length; i++) {
              args[1][i] += session.offset * 1e-3
            }
            return utils.cache.Reflect.apply(target, ctx, args)
          }
        }
        ...
        if (session.webgl2) {
          ...
          const getContextProxyHandler = {
            apply: function(target, ctx, args) {
              if (args[0].includes('webgl2')) return undefined
              return utils.cache.Reflect.apply(target, ctx, args)
            }
          }
          utils.replaceWithProxy(
