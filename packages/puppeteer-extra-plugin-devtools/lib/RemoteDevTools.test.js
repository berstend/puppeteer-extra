...
const { DevToolsCommon, DevToolsLocal, DevToolsTunnel } = require('./RemoteDevTools')
...
test('will throw when missing webSocketDebuggerUrl', async (t) => {
  ...
  t.is(error.message, "Expected argument to be of type `string` but received type `undefined`") // eslint-disable-line
})
...
test('DevToolsLocal: has basic functionality', async (t) => {
  ...
  t.is(instance.getUrlForPageId('foobar'), 'http://localhost:9222/devtools/inspector.html?ws=localhost:9222/devtools/page/foobar')
})
...
test('DevToolsTunnel: has basic functionality', async (t) => {
  ...
  t.is(instance.getUrlForPageId('foobar'), 'https://faketunnel.com/devtools/inspector.html?wss=faketunnel.com/devtools/page/foobar')
})
