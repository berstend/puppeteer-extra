...
test('will throw without opts', async (t) => {
  ...
  t.is(error.message, 'Expected argument to be of type `object` but received type `undefined`')
})

test('will throw when opts.obj is not a class derivative', async (t) => {
  const error = await t.throws(() => new REPLSession({ obj: 'foobar' }))
  t.is(error.message, 'Expected argument to be of type `object` but received type `string`')
})
